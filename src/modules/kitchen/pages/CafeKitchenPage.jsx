...
export default function CafeKitchenPage() {
  ...
  const fetchCafeOrders = async () => {
    try {
      setError("");
      const res = await api("/kitchen?kitchen_outlet_id=CAFE_KITCHEN");
      ...
      console.error("Failed to fetch cafe kitchen orders:", err);
      setError(err.message || "Failed to load cafe kitchen orders");
    } finally {
      setLoading(false);
    }
  };
  ...
  useEffect(() => {
    fetchCafeOrders();
    const interval = setInterval(fetchCafeOrders, 4000);
  ...
  return (
    <div className="space-y-6">
      {/* Station Banner */}
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between rounded-2xl bg-gradient-to-r from-amber-900 via-amber-800 to-amber-950 p-6 text-white shadow-md">
        <div>
          <div className="flex items-center gap-2">
            <span className="flex items-center gap-1.5 rounded-full bg-amber-500/20 px-3 py-1 text-xs font-bold text-amber-300 border border-amber-400/30">
              <Coffee className="h-3.5 w-3.5" />
              Cafe Culinary Division
            </span>
          ...
          <h1 className="text-2xl sm:text-3xl font-black mt-2 tracking-tight">
            Cafe Kitchen Display (KDS)
          </h1>
          <p className="text-xs sm:text-sm text-amber-200/80 mt-1">
            Dedicated preparation screen for Cafe Chef &amp; Cafe Supervisor. Displays cafe snacks, breakfasts, pastries &amp; light orders.
          </p>
        </div>

        <div className="flex items-center gap-2 sm:gap-3">
          <button
            onClick={fetchCafeOrders}
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        ...
        >
          <span className="text-[11px] font-bold uppercase text-blue-600">In Preparation</span>
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between rounded-2xl border border-slate-200 bg-white p-3 sm:p-4 shadow-xs">
        ...
        <div className="relative w-full sm:max-w-xs">
          ...
          <input
            ...
            placeholder="Search cafe order #, table, item..."
      ...
      {loading ? (
        <div className="flex h-64 items-center justify-center rounded-2xl border border-dashed border-slate-200 bg-white text-sm font-semibold text-slate-400">
          Loading Cafe Kitchen queue...
        </div>
      ) : filteredOrders.length === 0 ? (
        <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-slate-200 bg-white py-16 text-center">
          <Coffee className="h-10 w-10 text-amber-300 mb-3" />
          <h3 className="text-base font-bold text-slate-700">No Orders in Cafe Kitchen</h3>
          <p className="text-xs text-slate-400 mt-1 max-w-sm">
            Orders placed from the Cafe station will appear here in real-time.
          </p>
        </div>
      ) : (
              ...
              >
                ...
                <div className="border-b border-slate-200/80 p-4 bg-white/70">
                  ...
                  <div className="flex items-center justify-between text-xs text-slate-400 mt-2">
                    ...
                    <span className="text-[11px] font-semibold text-amber-700">Cafe Station</span>
                  </div>

                  {order.notes && (
                    <div className="mt-2 rounded-lg bg-amber-100/70 p-2 text-xs font-semibold text-amber-900 border border-amber-200/60">
                      Note: {order.notes}
                    </div>
                  )}
                </div>
                ...
                <div className="border-t border-slate-200/80 p-3 bg-white/90">
                  {isPending && (
                    <button
                      ...
                      className="w-full flex items-center justify-center gap-1.5 rounded-xl bg-amber-600 px-3 py-2 text-xs font-bold text-white shadow-xs hover:bg-amber-500 transition cursor-pointer disabled:opacity-50"
                    >
